...
export default function PermissionMatrixUi() {
  ...
  const [matrix, setMatrix] =
    useState<Record<string, PermissionRow[]>>(INITIAL_MATRIX_STATE);
        ...
        <div className="flex-1 space-y-1">
          ...
          <Select value={selectedUser} onValueChange={setSelectedUser}>
            <SelectTrigger className="bg-card border-border focus:ring-brand text-foreground h-7 w-full rounded-md text-xs shadow-none focus:ring-1">
              <SelectValue
                placeholder={t("identifyTargetedOperationalIdentity")}
              />
            </SelectTrigger>
            <SelectContent className="text-xs">
              {MOCK_USERS.map((usr) => (
                <SelectItem key={usr.id} value={usr.id}>
                  {usr.name} —{" "}
                  <span className="opacity-60">{t(usr.roleKey)}</span>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
              ...
              <TableHeader className="bg-accent/40">
                <TableRow className="border-border border-b hover:bg-transparent">
                  ...
                  {PERMISSION_COLUMNS.map((col) => {
                    ...
                    const isAllChecked =
                      columnRows.length > 0 &&
                      columnRows.every((r) => r.permissions[col.key]);
                      ...
                      <TableCell key={col.key} className="py-2 text-center">
                        <div className="inline-flex items-center justify-center">
                          <Checkbox
                            ...
                            onCheckedChange={() =>
                              handleToggleCell(tabKey, row.id, col.key)
                            }
