...
export const masterContactsApi = {
  ...
  organizationsDropdown: {
    list: async () =>
      (
        await axiosClient.get<{ data: any[] }>('/master/contacts/individuals/common/dropdown', {
          params: { type: 'O' },
        })
      ).data.data,
    create: async () => ({}) as any,
    ...
    list: async (params?: {
      ...
      (
        await axiosClient.get<{
          data: IndividualRecord[];
          meta?: { total: number; page: number; limit: number };
        }>('/master/contacts/individuals', { params })
      ).data,
    ...
    update: async (id: string, data: Partial<OrganisationDto>): Promise<OrganisationRecord> =>
      (
        await axiosClient.put<{ data: OrganisationRecord }>(
          `/master/contacts/organisations/${id}`,
          withUserId(data as any),
        )
      ).data.data,
    remove: async (id: string): Promise<void> => {
      await axiosClient.delete(`/master/contacts/organisations/${id}`);
    },
  },
};
