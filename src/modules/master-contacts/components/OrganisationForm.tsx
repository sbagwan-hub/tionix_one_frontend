...
import { MapPin, Globe, Building2, User, Phone, Mail, FileText } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      ...
      <div className="space-y-4 lg:col-span-2">
        ...
        <div className="border-border/80 bg-background/50 space-y-3 rounded-sm border p-4">
          ...
          <div className="border-border/80 bg-background/40 overflow-hidden rounded-sm border">
            <table className="w-full border-collapse text-left text-xs">
              <thead className="bg-muted/80 text-muted-foreground font-semibold">
                <tr>
                  <th className="border-b px-3 py-2">Contact Person(s)</th>
                  <th className="border-b px-3 py-2">Department</th>
                  <th className="border-b px-3 py-2">Designation</th>
                  <th className="border-b px-3 py-2">Phone / Mobile</th>
                </tr>
              </thead>
              <tbody className="divide-border/60 divide-y">
                {contactPersons.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="text-muted-foreground py-4 text-center italic">
                      No individuals associated with this organisation.
                    </td>
                  </tr>
                ) : (
                  contactPersons.map((ind: any) => (
                    <tr key={ind.pk_ind_id} className="hover:bg-muted/20">
      ...
      <div className="space-y-4">
        ...
        {/* Reference(s) Sub-form (Premium Custom Visual Match) */}
        <div className="border-border/80 bg-background/50 space-y-2 rounded-sm border p-3.5">
          <Label className="text-muted-foreground block text-[10px] font-semibold tracking-wider uppercase">
            Reference(s) / Ledger Linkages
          </Label>
          <div className="border-border/80 bg-background/40 flex h-24 items-center justify-center rounded-sm border">
            <span className="text-muted-foreground text-[10px] italic">No active references</span>
          </div>
        </div>
      </div>
    </div>
