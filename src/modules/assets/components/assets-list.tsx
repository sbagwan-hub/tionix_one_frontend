...
interface AssetsListProps {
  ...
  load_data: () => void;
  on_select_record: (rec: Asset, index: number) => void;
  on_double_click_record: (rec: Asset, index: number) => void;
}
    ...
    return records.filter((r) => {
      ...
      const descMatch = filter_desc
        ? r.description.toLowerCase().includes(filter_desc.toLowerCase()) ||
          ...
          (r.account || '').toLowerCase().includes(filter_desc.toLowerCase())
        : true;
      return codeMatch && descMatch;
    });
  }, [records, filter_code, filter_desc]);

  const columns = React.useMemo<ColumnDef<Asset, any>[]>(
    () => [
      {
        id: 'index',
        header: '#',
